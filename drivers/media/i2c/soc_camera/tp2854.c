    ...
    unsigned char reg  , unsigned char *val)
{
	...
	pr_debug("addr 0x%x reg 0x%x value 0x%x\n", client->addr, reg, ret);
...
int tp2854_hardware_init(struct i2c_client *client)
{
    ...
    switch (priv->nxpl) {
        ...
        case NPXL_1080P_30:
            ...
            break;
        case NPXL_480I:
            tp2854_write_reg(client ,TP2854_SYNC_CLAMP_CTRL, 0x3c);
...
static int tp2854_parse_dt(struct i2c_client *client)
{
	...
	if (val != TP2854_ID_1) {
        dev_err(&client->dev, "tp2854 read TP2854_ID1 fail %x\n", val);
		return -ENODEV;
	}
    pr_debug("tp2854 read TP2854_ID1 %x\n", val);

    /* read TP2854_ID_2 */
	tp2854_read_reg(client, TP2854_ID2_REG, &val);
	if (val != TP2854_ID_2) {
        dev_err(&client->dev,  "tp2854 read TP2854_ID2 fail %x\n", val);
		return -ENODEV;
	}
    pr_debug("tp2854 read TP2854_ID2 %x\n", val);
				 ...
				 const struct i2c_device_id *did)
{
	...
	for (i = 0; i < CHANNEL_NUM; i++) {
		...
		pr_debug("v4l2_async_register_subdev ret %d",err);
		if (err < 0)
			goto out;
	}
